...
| **RAG pipeline** | Live-wired into Express chat via [`intentGate.js`](server/services/intentGate.js) → [`ragClient.js`](server/services/ragClient.js) → FastAPI `/rag/query` | 508-chunk real corpus (Tax, SEBI, RBI/DICGC), FAISS `IndexFlatIP` vector store, 70-query eval (`eval_questions_v2.json`): 100% source hit rate, Recall@4 1.0, MRR 1.0, 27.5ms avg latency |
